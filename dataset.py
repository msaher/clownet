...
from pycoviar.transforms import color_aug
...
class CoviarDataSet(data.Dataset):
    ...
    @staticmethod
    def get_dummy(representation: str, batch_size=2) -> np.ndarray:
        """
        return a dummy data point
        """

        if representation not in ['mv', 'residual', 'iframe']:
            raise ValueError(f"Unkown representation f{representation}")

        if representation == 'mv':
            arr = np.random.randn(batch_size, 2, 224, 224)
            zeros_channel = np.zeros((batch_size, 1, 224, 224))
            arr = np.concatenate((arr, zeros_channel), axis=1)
        else:
            arr = np.random.randn(batch_size, 3, 224, 224)
    ...
    def __getitem__(self, index):
        ...
        for seg in range(self._num_segments):
            ...
            # yo load() returns -> (width, height, channel).
            # a later call to np.transpose() will fix that
